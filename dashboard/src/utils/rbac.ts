...
export const canAccessRoute = (admin: AdminDetails | null | undefined, pathname: string) => {
  ...
  if (pathname.startsWith('/fleet')) return canReadResourcePage(admin, 'nodes') || hasPermission(admin, 'system', 'read')
  ...
  if (pathname.startsWith('/audit')) return hasPermission(admin, 'audit_logs', 'read')
