    ...
    <div className="relative flex min-h-screen w-full overflow-hidden">
      <aside className="auth-mesh relative hidden w-[46%] flex-col justify-between overflow-hidden p-10 text-white lg:flex xl:w-[48%]">
        ...
        <div className="relative z-10 max-w-md animate-rise space-y-5">
          ...
          <p className="max-w-sm text-sm leading-relaxed text-white/65">
            {t('login.welcomeBack', { defaultValue: 'Sign in to manage your infrastructure with a quieter, denser console.' })}
          </p>
          <div className="grid grid-cols-3 gap-3 pt-2">
            {[
              { label: 'Live ops', value: '24/7' },
              { label: 'Protocols', value: 'Multi' },
              { label: 'Access', value: 'RBAC' },
            ].map((item, index) => (
              <div
                key={item.label}
                className={cn('rounded-2xl border border-white/10 bg-white/5 px-3 py-3 backdrop-blur-sm', index === 0 && 'animate-rise', index === 1 && 'animate-rise animate-rise-delay-1', index === 2 && 'animate-rise animate-rise-delay-2')}
              >
                <p className="font-mono text-lg font-semibold text-white">{item.value}</p>
                <p className="mt-1 text-[10px] tracking-[0.14em] text-white/50 uppercase">{item.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <main className="relative flex min-h-screen flex-1 flex-col">
        ...
        <div className="flex flex-1 items-center justify-center px-5 py-10 sm:px-8">
          <div className="surface-panel animate-rise w-full max-w-[420px] rounded-[1.75rem] p-6 sm:p-8">
            ...
            {view === 'login' ? (
              <form onSubmit={handleSubmit(handleLogin)} autoComplete="on" className="space-y-3">
                ...
                <div className="flex flex-col gap-2 pt-2">
                  <LoaderButton isLoading={loading || miniAppLoading || telegramLoading} type="submit" className="flex h-12 w-full items-center gap-2">
                    <LogInIcon size="18px" />
                    <span>{t('login')}</span>
                  </LoaderButton>
                  <Button type="button" variant="outline" className="flex h-11 w-full items-center gap-2" onClick={switchToSetup}>
              ...
              <form className="flex flex-col gap-3" onSubmit={handleOwnerSubmit(onOwnerSubmit)} autoComplete="off">
                ...
                <Tabs value={ownerSetupMode} onValueChange={handleOwnerSetupModeChange} className="w-full">
                  <TabsList className="grid h-auto w-full grid-cols-2 gap-1 rounded-2xl p-1">
                    <TabsTrigger value="create" className="h-9 gap-1 rounded-xl px-2 text-xs">
                      <ShieldCheck className="h-3.5 w-3.5 shrink-0" />
                      <span className="truncate">{t('setup.createOwnerShort', { defaultValue: 'Create' })}</span>
                    </TabsTrigger>
                    <TabsTrigger value="upgrade" className="h-9 gap-1 rounded-xl px-2 text-xs">
                    ...
                    <TabsTrigger value="reset" className="h-9 gap-1 rounded-xl px-2 text-xs">
                      <RotateCcw className="h-3.5 w-3.5 shrink-0" />
                      <span className="truncate">{t('setup.resetOwnerShort', { defaultValue: 'Reset' })}</span>
                    </TabsTrigger>
                    <TabsTrigger value="delete" className="h-9 gap-1 rounded-xl px-2 text-xs">
                ...
                {ownerSetupMode === 'delete' && (
                  <>
                    ...
                    />
                    <Alert variant="destructive" className="mt-1 py-4">
                      <CircleAlertIcon size="18px" />
                ...
                <div className="mt-1 flex flex-col gap-2">
                  ...
                  <Button type="button" variant="ghost" className="flex h-11 w-full items-center gap-2" onClick={switchToLogin}>
