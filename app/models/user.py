...
class User(BaseModel):
    ...
    ip_limit: int | None = Field(
        default=None,
        ge=0,
        description="Max concurrent unique IPs for this user. Empty = unlimited. Example: 2 blocks a 3rd IP.",
    )
...
class UserNotificationResponse(User):
    ...
    @classmethod
    def normalize_group_quotas(cls, value):
        if value is None:
            return []
                ...
                return [
                    {
                        ...
                        "group_name": getattr(getattr(quota, "group", None), "name", None),
