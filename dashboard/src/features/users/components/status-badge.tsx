...
import { WifiSignalIcon } from './wifi-signal-icon'
...
export const StatusBadge = memo(({ expiryDate = null, status: userStatus, showExpiry, showOnlyExpiry, lastOnline }: UserStatusProps) => {
  ...
  const isOnline = isActive && isUserOnline(lastOnline)
    ...
    <div className={cn('flex flex-wrap justify-start gap-x-2')}>
      ...
      >
        <div className={cn('flex items-center gap-1 sm:px-1', showExpiry && 'px-1')}>
          {isOnline ? <WifiSignalIcon animated className="text-current" /> : StatusIcon ? <StatusIcon className="h-4 w-4 sm:h-3 sm:w-3" /> : null}
