...
def upgrade() -> None:
    with op.batch_alter_table("hpx_tunnels", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("auto_heal_enabled", sa.Boolean(), nullable=False, server_default=sa.text("1"))
        )
        ...
        batch_op.add_column(
            sa.Column("heal_count_window", sa.Integer(), nullable=False, server_default=sa.text("0"))
        )
