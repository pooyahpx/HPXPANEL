          ...
          <PopoverTrigger asChild>
            <Button variant="outline" className={cn('hover:bg-muted/50 h-8 w-full justify-between px-2 transition-colors sm:h-9 sm:px-3', 'min-w-0 text-xs font-medium sm:text-sm')}>
              <div className={cn('flex min-w-0 flex-1 items-center gap-1 sm:gap-2', dir === 'rtl' ? 'flex-row-reverse' : 'flex-row')}>
                ...
                {selectedCore?.type && <span className="text-muted-foreground shrink-0 text-[10px]">{t(`coreTypes.${String(selectedCore.type)}`, { defaultValue: String(selectedCore.type).toUpperCase() })}</span>}
              </div>
              ...
              <CommandList ref={listRef}>
                ...
                {cores.map((core: any) => (
                  ...
                  >
                    ...
                    <span className="text-muted-foreground shrink-0 text-[10px]">{t(`coreTypes.${String(core.type ?? 'xray')}`, { defaultValue: String(core.type ?? 'xray').toUpperCase() })}</span>
