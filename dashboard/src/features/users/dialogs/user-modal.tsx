...
function UserModal({ isDialogOpen, onOpenChange, form, editingUser, editingUserId, editingUserData, onSuccessCallback }: UserModalProps) {
  ...
  const [activeStep, setActiveStep] = useState<'identity' | 'access' | 'limits' | 'advanced'>('identity')
    ...
    async (values: UseFormValues | UseEditFormValues) => {
      try {
        ...
        const preparedValues = {
          ...
          ip_limit: typeof values.ip_limit === 'string' ? parseFloat(values.ip_limit) : values.ip_limit,
        ...
        const sendValues: any = {
          ...
          ip_limit: preparedValues.ip_limit == null ? null : Number.isFinite(Number(preparedValues.ip_limit)) ? Math.round(Number(preparedValues.ip_limit)) : null,
  ...
  const watchedStepUsername = form.watch('username')
  const watchedStepGroupIds = form.watch('group_ids')
  const watchedStepDataLimit = form.watch('data_limit')
  const watchedStepExpire = form.watch('expire')
  const wizardSteps: { id: 'identity' | 'access' | 'limits' | 'advanced'; label: string; hint: string; done: boolean }[] = [
    {
      id: 'identity',
      label: t('userDialog.stepIdentity', { defaultValue: 'Identity' }),
      hint: t('userDialog.stepIdentityHint', { defaultValue: 'User · status' }),
      done: !!watchedStepUsername && watchedStepUsername.length >= 3,
    },
    {
      id: 'access',
      label: t('userDialog.stepAccess', { defaultValue: 'Access' }),
      hint: t('userDialog.stepAccessHint', { defaultValue: 'Groups · template' }),
      done: (watchedStepGroupIds?.length ?? 0) > 0 || !!selectedTemplateId,
    },
    {
      id: 'limits',
      label: t('userDialog.stepLimits', { defaultValue: 'Limits' }),
      hint: t('userDialog.stepLimitsHint', { defaultValue: 'Quota · expiry · IP' }),
      done: Number(watchedStepDataLimit) > 0 || !!watchedStepExpire,
    },
    {
      id: 'advanced',
      label: t('userDialog.stepAdvanced', { defaultValue: 'Advanced' }),
      hint: t('userDialog.stepAdvancedHint', { defaultValue: 'Note · proxy' }),
      done: false,
    },
  ]
  const wizardStepIndex = Math.max(
    0,
    wizardSteps.findIndex(s => s.id === activeStep),
  )

  return (
    <Dialog open={isDialogOpen} onOpenChange={handleModalOpenChange}>
      <DialogContent className="command-surface h-auto gap-0 overflow-hidden border p-0 shadow-[6px_6px_0_hsl(var(--pixel-border))] lg:min-w-[1024px]">
        <DialogHeader className="mission-brief relative border-b px-5 py-4 sm:px-6">
          <div className="mission-brief__index" aria-hidden="true">
            {editingUser ? 'EU' : 'CU'}
          </div>
          <DialogTitle className="relative z-10 flex items-center gap-2.5">
            <span className="bg-primary text-primary-foreground flex h-8 w-8 items-center justify-center border border-[hsl(var(--pixel-border))] shadow-[2px_2px_0_hsl(var(--pixel-border))]">
              {editingUser ? <Pencil className="h-4 w-4" /> : <UserRoundPlus className="h-4 w-4" />}
            </span>
          ...
          <DialogDescription className="relative z-10 mt-1 font-mono text-[10px] font-bold tracking-[0.16em] uppercase">
            {editingUser ? 'Identity control' : 'Provision new identity'}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-0">
            <div className="flex max-h-[80dvh] w-full flex-col overflow-hidden sm:max-h-[75dvh] lg:h-[560px] lg:flex-row">
              {/* Wizard step rail */}
              <nav
                className="border-border bg-card/30 flex shrink-0 gap-1 overflow-x-auto border-b p-2 lg:w-56 lg:flex-col lg:gap-0 lg:overflow-visible lg:border-e lg:border-b-0 lg:p-0"
                aria-label={t('userDialog.steps', { defaultValue: 'Steps' })}
              >
                {wizardSteps.map((step, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'flex min-w-36 flex-1 items-center gap-3 border px-3 py-2 text-start transition-colors lg:min-w-0 lg:flex-none lg:border-x-0 lg:border-t-0 lg:border-b lg:px-4 lg:py-4',
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          isActive ? 'border-primary bg-primary text-primary-foreground shadow-[2px_2px_0_hsl(var(--pixel-border))]' : step.done ? 'border-primary/50 text-primary' : 'border-border',
                        )}
                        aria-hidden="true"
                      >
                        {step.done && !isActive ? '✓' : `0${i + 1}`}
                      </span>
                      <span className="min-w-0">
                        <span className="block truncate text-xs font-bold">{step.label}</span>
                        <span className="hidden truncate font-mono text-[9px] tracking-[0.12em] uppercase opacity-70 lg:block">{step.hint}</span>
                      </span>
                ...
                <div className="border-border hidden border-t px-4 py-3 lg:block">
                  <p className="text-muted-foreground font-mono text-[9px] font-bold tracking-[0.13em] uppercase">{t('userDialog.liveDraft', { defaultValue: 'Live draft' })}</p>
                  <p className="mt-1 truncate text-xs font-bold">{watchedStepUsername || '—'}</p>
                  <p dir="ltr" className="text-muted-foreground mt-0.5 font-mono text-[10px] tabular-nums">
                    {watchedStepGroupIds?.length ?? 0} groups · {Number(watchedStepDataLimit) > 0 ? `${watchedStepDataLimit} GB` : '∞'}
                  </p>
                </div>
              </nav>
              {/* Wizard step content */}
              <div className="min-w-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5">
                <div className={cn('space-y-4', activeStep === 'identity' ? 'animate-rise block' : 'hidden')}>
                  <div className="border-border bg-card/40 space-y-4 border p-4">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-primary font-mono text-[10px] font-bold tracking-[0.14em] uppercase">01 / Identity</p>
                ...
                <div className={cn('space-y-4', activeStep === 'limits' ? 'animate-rise block' : 'hidden')}>
                  {/* Data limit and expire fields - show data_limit only when no template is selected */}
                  {activeTab === 'groups' && (
                    <div className="border-border bg-card/40 space-y-4 border p-4">
                      <p className="text-primary font-mono text-[10px] font-bold tracking-[0.14em] uppercase">03 / Quotas & Limits</p>
                    <div className="flex w-full flex-col gap-4">
                      ...
                      {!selectedTemplateId && (
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="relative w-full min-w-0">
                              <FormLabel>{t('userDialog.ipLimit', { defaultValue: 'IP Limiter' })}</FormLabel>
                              <FormControl>
                                <Input
                                  placeholder={t('userDialog.ipLimitPlaceholder', { defaultValue: 'Empty = unlimited, e.g. 2' })}
                                  value={field.value ?? ''}
                                  inputMode="numeric"
                                  pattern="[0-9]*"
                                  onBlur={field.onBlur}
                              ...
                              <FormDescription className="text-xs">
                                {t('userDialog.ipLimitHint', {
                                  defaultValue: 'Max concurrent unique IPs for this config. Extra IPs beyond this limit cannot connect. Empty = unlimited.',
                                })}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                ...
                <div className={cn('space-y-4', activeStep === 'advanced' ? 'animate-rise block' : 'hidden')}>
                  <div className="border-border bg-card/40 space-y-4 border p-4">
                    <p className="text-primary font-mono text-[10px] font-bold tracking-[0.14em] uppercase">04 / Notes & Proxy</p>
                  <FormField
                    control={form.control}
                    name="note"
                    render={({ field }) => (
                  ...
                  {activeTab === 'groups' && (
                    <Accordion type="single" collapsible className="w-full">
                      <AccordionItem className="rounded-none border px-4 [&_[data-state=closed]]:no-underline [&_[data-state=open]]:no-underline" value="proxySettings">
                        <AccordionTrigger>
                          <div className="flex items-center gap-2">
                        ...
                        <AccordionContent className="px-2">
                          ...
                          />
                          <div className="bg-muted/20 mb-3 space-y-3 rounded-sm border p-3">
                            <div>
                              <div className="text-sm font-medium">{t('userDialog.proxySettings.ipsecSharedTitle')}</div>
                              <p className="text-muted-foreground mt-1 text-xs">{t('userDialog.proxySettings.ipsecSharedDescription')}</p>
                            </div>
                            <FormField
                              control={form.control}
                              name="proxy_settings.ikev2.username"
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>{t('userDialog.proxySettings.ipsecUsername')}</FormLabel>
                                  <FormControl>
                                    <Input
                                      dir="ltr"
                                      autoComplete="off"
                                      {...field}
                                      value={field.value ?? ''}
                                      placeholder={t('userDialog.proxySettings.ipsecUsernamePlaceholder')}
                                      onChange={event => {
                                        field.onChange(event)
                                        form.trigger('proxy_settings.ikev2.username')
                                        handleFieldChange('proxy_settings.ikev2.username', event.target.value)
                                      }}
                                    />
                                  </FormControl>
                                  <FormMessage />
                            ...
                            <FormField
                              control={form.control}
                              name="proxy_settings.ikev2.password"
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>{t('userDialog.proxySettings.ipsecPassword')}</FormLabel>
                                  <FormControl>
                                    <div dir="ltr" className={`flex items-center gap-2 ${dir === 'rtl' ? 'flex-row-reverse' : 'flex-row'}`}>
                                      <PasswordInput
                                        ...
                                        {...field}
                                        value={field.value ?? ''}
                                        placeholder={t('userDialog.proxySettings.ipsecPasswordPlaceholder')}
                                        onChange={event => {
                                          field.onChange(event)
                                          form.trigger('proxy_settings.ikev2.password')
                                          handleFieldChange('proxy_settings.ikev2.password', event.target.value)
                                        }}
                                      />
                                      <Button
                                        size="icon"
                                        ...
                                        onClick={() => {
                                          ...
                                          field.onChange(next)
                                          form.trigger('proxy_settings.ikev2.password')
                                          handleFieldChange('proxy_settings.ikev2.password', next)
                                        }}
                                        title={t('userDialog.proxySettings.generateIpsecPassword')}
                                      >
                                        <RefreshCcw className="h-3 w-3" />
                                      </Button>
                                    </div>
                  ...
                  {activeTab === 'groups' && editingUser && canUseNextPlan && (
                    <div className="border-border bg-card/40 rounded-none border p-4">
                ...
                <div className={cn('space-y-4', activeStep === 'access' ? 'animate-rise block' : 'hidden')}>
                <div className="border-border bg-card/40 w-full min-w-0 space-y-3 border p-4">
                  <p className="text-primary font-mono text-[10px] font-bold tracking-[0.14em] uppercase">02 / Assignment</p>
                  <div className="w-full">
            ...
            {/* Wizard navigation + Cancel/Create */}
            <div className="border-border bg-background/80 flex flex-row items-center gap-3 border-t px-4 py-3 sm:px-5">
              <div className="text-muted-foreground me-auto hidden items-center gap-2 font-mono text-[10px] font-bold tracking-[0.12em] uppercase sm:flex">
                <span className="text-primary">{String(wizardStepIndex + 1).padStart(2, '0')}</span>
                <span aria-hidden="true">/</span>
                {String(wizardSteps.length).padStart(2, '0')}
                <span className="hidden md:inline">· {wizardSteps[wizardStepIndex]?.label}</span>
              </div>
              <Button type="button" variant="outline" size="sm" disabled={wizardStepIndex === 0} onClick={() => setActiveStep(wizardSteps[wizardStepIndex - 1].id)}>
                {t('back', { defaultValue: 'Back' })}
              </Button>
              {wizardStepIndex < wizardSteps.length - 1 && (
                <Button type="button" variant="secondary" size="sm" onClick={() => setActiveStep(wizardSteps[wizardStepIndex + 1].id)}>
