...
import { IpsecCoreEditor } from '@/features/core-editor/components/ipsec/ipsec-core-editor'
...
function CoreEditorLoadingSkeleton({ coreKind }: { coreKind?: LoadingCoreKind }) {
  ...
  const formLike = neutral || coreKind !== 'xray'
...
export default function CoreEditorPage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (preSaveIssues.length > 0) {
      const firstIssue = preSaveIssues[0]
      toast.error(firstIssue ? `${firstIssue.issue.path}: ${firstIssue.issue.message}` : t('coreEditor.validationErrors', { defaultValue: 'Validation errors' }))
      return
    }
    setSaving(true)
    try {
                ...
                if (isNew) {
                  setSearchParams(
                    prev => {
                      ...
                      if (nextKind !== 'xray') p.set('kind', nextKind)
                      else p.delete('kind')
                      return p
                    },
                    { replace: true },
  ...
  const sectionHeaderConfig = useMemo(() => {
    ...
    if (kind === 'ikev2' || kind === 'l2tp' || kind === 'openvpn') {
      ...
      return {
        configuration: {
          title: 'coreEditor.section.configuration',
          description: `coreEditor.sectionDesc.${kind}`,
        },
        advanced: {
          title: 'coreEditor.section.advanced',
          description: 'coreEditor.sectionDesc.advanced',
  ...
  if (!hydrated && !isNew && validId) {
    return (
      <div className="flex min-h-0 flex-1 flex-col">
        <CoreEditorLoadingSkeleton coreKind={(coreData?.type as DashboardCoreKind | undefined) ?? 'xray'} />
      </div>
    )
  }
