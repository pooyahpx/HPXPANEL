...
export const ikev2SettingsSchema = z
  .object({
    username: z.string().trim().min(1).max(128).optional(),
    password: z.string().min(2).max(128).optional(),
  })
  .superRefine((settings, ctx) => {
    if (settings.username && !settings.password) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'validation.required', path: ['password'] })
    }
    if (settings.password && !settings.username) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'validation.required', path: ['username'] })
    }
  })
export const proxyTableInputSchema = z.object({
  ...
  // L2TP intentionally reuses this exact credentials object.
  ikev2: ikev2SettingsSchema.optional(),
})
