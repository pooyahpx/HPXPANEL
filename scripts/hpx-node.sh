#!/usr/bin/env bash
#
# HPXPANEL Node — command-deck edge installer
# ------------------------------------------
...
# you paste into HPXPANEL → Nodes.
#
# One-liner (Linux):
#   sudo bash -c "$(curl -fsSL https://github.com/pooyahpx/HPXPANEL/raw/main/scripts/hpx-node.sh)" @ install
#
# Interactive menu:
#   sudo bash -c "$(curl -fsSL https://github.com/pooyahpx/HPXPANEL/raw/main/scripts/hpx-node.sh)"
#
# Local:
...
# the "@" is $0-style arg noise from PG-style one-liners — strip it.
if [ "${1:-}" = "@" ]; then shift; fi
...
FALLBACK_IMAGE="${FALLBACK_IMAGE:-pasarguard/node:latest}"
BRANCH="${BRANCH:-main}"
...
CONTAINER_DATA="/var/lib/pg-node"
...
banner() {
  ...
  echo -e "  ${c_cyn}${c_bld}┃${c_off}  ${c_amb}${c_bld}HPX${c_off}${c_bld}PANEL${c_off}  ${c_dim}//${c_off}  ${c_cyn}NODE DEPLOY${c_off}  ${c_dim}· COMMAND DECK${c_off}     ${c_cyn}${c_bld}┃${c_off}"
  echo -e "  ${c_cyn}${c_bld}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${c_off}"
  echo
  echo -e "  ${c_dim}Edge node for${c_off} ${c_bld}HPXPANEL${c_off} ${c_dim}— paste Address / Port / API key / CA into the panel.${c_off}"
  echo
}
...
features_panel() {
  ...
  echo -e "  ${c_dim}┌────────────────────────────────────────────────────┐${c_off}"
  echo -e "  ${c_dim}│${c_off}  ${c_cyn}◆${c_off} Xray stack     ${c_dim}VLESS / VMess / Trojan / SS / REALITY${c_off}"
  echo -e "  ${c_dim}│${c_off}  ${c_cyn}◆${c_off} WireGuard      ${c_dim}kernel WG + host NAT routing${c_off}"
  echo -e "  ${c_dim}│${c_off}  ${c_cyn}◆${c_off} OpenVPN        ${c_dim}optional tunnel backend${c_off}"
  echo -e "  ${c_dim}│${c_off}  ${c_amb}◆${c_off} IKEv2 / IPsec  ${c_dim}native strongSwan (HPX special)${c_off}"
  echo -e "  ${c_dim}│${c_off}  ${c_grn}◆${c_off} Panel sync     ${c_dim}gRPC → HPXPANEL Nodes UI${c_off}"
  echo -e "  ${c_dim}└────────────────────────────────────────────────────┘${c_off}"
  echo
}
...
press_enter() { echo; _read -r -p "$(echo -e "  ${c_dim}Press Enter to return to the deck…${c_off}")" _; }
...
menu_command() {
  ...
  while true; do
    ...
    echo
    local choice
    _read -r -p "$(echo -e "  ${c_bld}Select${c_off} ${c_dim}(number / letter)${c_off} ${c_amb}❯${c_off} ")" choice
    case "$choice" in
      1) XRAY_ON=$((1 - XRAY_ON)) ;;
      2) OVPN_ON=$((1 - OVPN_ON)) ;;
      3) WG_ON=$((1 - WG_ON)) ;;
